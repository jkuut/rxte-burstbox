...
burst_compress = false #compress burst folders
data_compress = true #compress data folders (PXXXXX)
    ...
    if source in good_sources
        ...
        if burst_compress 
            ...
            for bf in bfolders
                ...
                #zip
                eval(parse("run(`tar -caf proc_sum_fit.tar.lzo $(tarf)`)"))
                #exit()
                
                #remove dirs after zipping is complete
                if stat("proc_sum_fit.tar.lzo").size > 0
                    for psf in ["proc", "sum", "fit"]
                        if stat(psf).size > 0
                            eval(parse("run(`rm -r $(psf)`)"))
                        end
                    end
                end
